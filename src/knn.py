...
def score_candidate_pool(ref_vector, candidate_pool_data, *, exploration_constant=1.25):
    ...
    for candidate in candidate_pool_data:
        ...
        if visits == 0:
            ...
            ucb_score = q_value + exploration_constant * math.sqrt(
                math.log(total_tries) / visits
            )
...
def record_feedback(candidate, thumbs_up):
    ...
    if previous_n <= 0:
        ...
        bandit_record["q"] = (
            (bandit_record["q"] * previous_n) + reward
        ) / bandit_record["n"]
...
def create_vector(
    ...
    vector.append(
        round((max_rank - rank) / max_rank, 4) * W_TIEBREAKER if max_rank > 0 else 0
    )

    # 6. Normalized Popularity
    pop = anime_data.get("Popularity") or max_pop
    vector.append(
        round((max_pop - pop) / max_pop, 4) * W_TIEBREAKER if max_pop > 0 else 0
    )
